...
public class CrcGeneric extends AbstractChecksum {
     ...
     */
    public CrcGeneric(final int width, final long poly, final long initialValue, final boolean refIn, final boolean refOut, final long xorOut) throws NoSuchAlgorithmException {
        super();
        this.width = width;
        this.poly = poly;
     ...
     * @throws java.security.NoSuchAlgorithmException if the parameter cannot be used to create a correct object
     */
    public CrcGeneric(final String props) throws NoSuchAlgorithmException {
        final String[] array = props.split(","); // we need compatibility with JRE 1.3
        if (array.length != 6) {
        ...
        try {
            ...
            initialValue = new BigInteger(array[2], 16).longValue(); //Long.parseLong(array[2], 16);
            refIn = "true".equalsIgnoreCase(array[3]);
            refOut = "true".equalsIgnoreCase(array[4]);
            xorOut = new BigInteger(array[5], 16).longValue();
     ...
     */
    private static long reflect(long value, final int bits) {
        long temp = 0L;
    ...
    public String getString() {
        ...
        final int nibbles = width / 4 + ((width % 4 > 0) ? 1 : 0);
        sb.append(width);
        sb.append(',');
    ...
    public String getName() {
        return name == null ? getString() : name;
    }
    ...
    public byte[] getByteArray() {
        ...
        final byte[] array = new byte[width / 8 + ((width % 8 > 0) ? 1 : 0)];
