...
public abstract class BaseHash implements IMessageDigest {
    ...
    public void update(final byte b) {
        ...
        final int i = (int) (count % blockSize);
        count++;
        buffer[i] = b;
        if (i == (blockSize - 1)) {
            transform(buffer, 0);
    ...
    @Override
    public byte[] digest() {
        final byte[] tail = padBuffer(); // pad remaining bytes in buffer
        update(tail, 0, tail.length); // last transform of a message
        final byte[] result = getResult(); // make a result out of context
