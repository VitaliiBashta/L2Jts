...
public class Edonkey extends AbstractChecksum {
    ...
    public void update(final byte[] buffer, final int offset, final int len) {
        final int zuSchreiben = len - offset; // XXX
        final int passed = (int) (length % BLOCKSIZE);
        final int platz = BLOCKSIZE - passed;
    ...
    public byte[] getByteArray() {
        if (virgin) {
            if (length < BLOCKSIZE) {
                ...
                final IMessageDigest md4temp = (IMessageDigest) md4final.clone();
