 ...
 * @since 19.02.2016
 */
public class NameColorService extends CommunityBoardService
{
	...
	public void content(final Player player, final String bypass, final Object... params)
	{
		...
		if(str.length < 5)
		{
			player.sendPacket(new CustomMessage("bbs.service.nameColor.contentError"));
		...
		String htm = getCache().getHtml(CBasicConfig.BBS_PATH + "/cabinet/service/player/color/name_color.htm", player);
		...
		for(; correctLine <= countLine; )
		{
			...
			for(; correctCount <= maxPageColor; )
			{
				...
				final String generateButton = button.replace("<?color?>", "<a action=\"bypass _bbsservice:service:name_color:request:" + correctCount + "\"><font color=\"" + color.getColor() + "\">" + color.getName(player) + "</font></a><br>");
	...
	public void request(final Player player, final String bypass, final Object... params)
	{
		...
		if(str.length < 5)
		{
			player.sendPacket(new CustomMessage("bbs.service.nameColor.error"));
			useSaveCommand(player);
			return;
		}
		if(str[4].equalsIgnoreCase("reset"))
		{
			if(player.getAppearanceComponent().getNameColor() != AppearanceComponent.DEFAULT_NAME_COLOR)
			{
				reply(player, "FFFFFF");
				return;
			}
			...
			{
				player.sendMessage(new CustomMessage("bbs.service.nameColor.standartColor"));
				useSaveCommand(player);
				return;
			}
		}
		final int colorIndex = Converter.convert(Integer.class, str[4]);
	...
	public void reply(final Player player, final Object... params)
	{
		...
		player.getAppearanceComponent().setNameColor(Integer.decode("0x" + Util.RGBtoBGR(color)));
		player.broadcastUserInfo(true);
		useSaveCommand(player);
		player.sendPacket(new CustomMessage("bbs.service.successfully"));
		Log.service("NameColorService", player, "changed name color to " + color + ".");
	}

	@Override
	public Services getService()
	{
		return Services.name_color;
	}
