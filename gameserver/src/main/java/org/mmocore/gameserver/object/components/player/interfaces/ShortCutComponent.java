...
public class ShortCutComponent {
    private static final Logger _log = LoggerFactory.getLogger(ShortCutComponent.class);

    private final Player playerRef;
    private final Map<Integer, ShortCut> _shortCuts = new ConcurrentHashMap<>();

    public ShortCutComponent(final Player player) {
        playerRef = player;
    }
    ...
    public void validate() {
        ...
        // Удаляем ярлыки на предметы, которых нету в инвентаре
        {
            if (sc.getType() == ShortCut.TYPE_ITEM) {
                if (getPlayer().getInventory().getItemByObjectId(sc.getId()) == null) {
    ...
    public ShortCut getShortCut(int slot, int page) {
        ...
        if (sc != null && sc.getType() == ShortCut.TYPE_ITEM) {
            if (getPlayer().getInventory().getItemByObjectId(sc.getId()) == null) {
                getPlayer().sendPacket(SystemMsg.THERE_ARE_NO_MORE_ITEMS_IN_THE_SHORTCUT);
                deleteShortCut(sc.getSlot(), sc.getPage());
                sc = null;
            }
        }
    ...
    private synchronized void registerShortCutInDb(ShortCut shortcut, ShortCut oldShortCut) {
        ...
        try {
            ...
            statement.setInt(8, getPlayer().getPlayerClassComponent().getActiveClassId());
            statement.execute();
        } catch (Exception e) {
    ...
    private void deleteShortCutFromDb(ShortCut shortcut) {
        ...
        try {
            ...
            statement.setInt(2, shortcut.getSlot());
            statement.setInt(3, shortcut.getPage());
            statement.setInt(4, getPlayer().getPlayerClassComponent().getActiveClassId());
            statement.execute();
        } catch (Exception e) {
    ...
    public void restore() {
        ...
        try {
            ...
            statement.setInt(1, getPlayer().getObjectId());
            statement.setInt(2, getPlayer().getPlayerClassComponent().getActiveClassId());
