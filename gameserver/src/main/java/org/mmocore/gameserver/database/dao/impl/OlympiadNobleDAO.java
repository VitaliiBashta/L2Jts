 ...
 */
public class OlympiadNobleDAO extends AbstractGameServerDAO {
    ...
    public static final String OLYMPIAD_GET_PTS = "SELECT olympiad_points FROM olympiad_nobles WHERE char_id=?";
    public static final String OLYMPIAD_SET_PTS = "UPDATE olympiad_nobles SET olympiad_points=? WHERE char_id=?";
    ...
    public void replace(final int nobleId) {
        Connection con = null;
        PreparedStatement statement = null;
        try {
            con = DatabaseFactory.getInstance().getConnection();
    ...
    public int getPoints(int objId) {
        try {
            return jdbcHelper.queryForInt(OLYMPIAD_GET_PTS, objId);
        } catch (Exception e) {
            return 0;
        }
    }

    public void setPoints(int objId, int points) {
        jdbcHelper.execute(OLYMPIAD_SET_PTS, points, objId);
    }
}
