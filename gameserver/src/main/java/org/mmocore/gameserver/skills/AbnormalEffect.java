...
public enum AbnormalEffect {
    ...
    AVE_SPEED_DOWN("speed_down", 0x20000000, AbnormalEffectType.SECOND), // взят с ЖДС, что делает - непонятно.
    // ave_mp_shield
    ...
    D_GRADE_ENCHANT_WEAPON("d_grade_enchant_weapon", 0x00200000, AbnormalEffectType.SECOND),
    ...
    AbnormalEffect(final String name, final int mask, final AbnormalEffectType t) {
        _name = name;
        _mask = mask;
        _type = t;
    }
    ...
    public static AbnormalEffect getByName(final String name) {
        for (final AbnormalEffect eff : AbnormalEffect.values()) {
