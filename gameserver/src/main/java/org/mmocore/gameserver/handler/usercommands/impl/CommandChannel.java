package org.mmocore.gameserver.handler.usercommands.impl;

import org.mmocore.gameserver.handler.usercommands.IUserCommandHandler;
import org.mmocore.gameserver.model.team.Party;
import org.mmocore.gameserver.network.lineage.components.CustomMessage;
import org.mmocore.gameserver.network.lineage.components.SystemMsg;
import org.mmocore.gameserver.network.lineage.serverpackets.ExMultiPartyCommandChannelInfo;
import org.mmocore.gameserver.network.lineage.serverpackets.SystemMessage;
import org.mmocore.gameserver.object.Player;

/**
 * Support for CommandChannel commands:<br>
 * 92	/channelcreate<br>
 * 93	/channeldelete<br>
 * 94	/channelinvite [party leader] отправляет пакет RequestExMPCCAskJoin<br>
 * 95	/channelkick [party leader] отправляет пакет RequestExMPCCExit<br>
 * 96	/channelleave<br>
 * 97	/channelinfo<br>
 *
 * @author SYS
 * @author Java-man
 */
public class CommandChannel implements IUserCommandHandler {
    private static final int[] COMMAND_IDS = {92, 93, 96, 97};

    @Override
    public boolean useUserCommand(final int id, final Player activeChar) {
        if (id != COMMAND_IDS[0] && id != COMMAND_IDS[1] && id != COMMAND_IDS[2] && id != COMMAND_IDS[3]) {
            return false;
        }

        switch (id) {
            case 92: //channelcreate
                // "Используйте команду /channelinvite"
                activeChar.sendMessage(new CustomMessage("usercommandhandlers.CommandChannel"));
                break;
            case 93: //channeldelete
                if (!activeChar.isInParty() || !activeChar.getParty().isInCommandChannel()) {
                    return true;
                }
                final org.mmocore.gameserver.model.team.CommandChannel commandChannel = activeChar.getParty().getCommandChannel();
                final Player channelLeader = commandChannel.getGroupLeader();
                if (channelLeader == activeChar) {
                    commandChannel.askLeaderAboutDisbandChannel();
                    return true;
                } else {
                    activeChar.sendPacket(SystemMsg.ONLY_THE_CREATOR_OF_A_CHANNEL_CAN_USE_THE_CHANNEL_DISMISS_COMMAND);
                }
                break;
            case 96: //channelleave
                if (!activeChar.isInParty() || !activeChar.getParty().isInCommandChannel()) {
                    return true;
                }

                if (!activeChar.getParty().isLeader(activeChar)) {
                    activeChar.sendPacket(SystemMsg.ONLY_A_PARTY_LEADER_CAN_CHOOSE_THE_OPTION_TO_LEAVE_A_CHANNEL);
                    return true;
                }

                final org.mmocore.gameserver.model.team.CommandChannel channel = activeChar.getParty().getCommandChannel();

                // если Лидер СС покидает СС, то вся СС распускается
                if (channel.getGroupLeader() == activeChar) {
                    channel.askLeaderAboutDisbandChannel();
                    return true;
                }

                final Party party = activeChar.getParty();
                channel.removeParty(party);
                party.broadCast(SystemMsg.YOU_HAVE_QUIT_THE_COMMAND_CHANNEL);
                channel.broadCast(new SystemMessage(SystemMsg.S1_PARTY_HAS_LEFT_THE_COMMAND_CHANNEL).addString(activeChar.getName()));
                break;
            case 97: //channelinfo
                if (!activeChar.isInParty() || !activeChar.getParty().isInCommandChannel()) {
                    return false;
                }
                activeChar.sendPacket(new ExMultiPartyCommandChannelInfo(activeChar.getParty().getCommandChannel()));
                break;
        }
        return true;
    }

    @Override
    public final int[] getUserCommandList() {
        return COMMAND_IDS;
    }
}
