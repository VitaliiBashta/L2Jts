...
import static org.jts.dataparser.data.holder.setting.common.PlayerRace.*;
import static org.mmocore.gameserver.model.base.ClassLevel.*;
import static org.mmocore.gameserver.model.base.ClassType.*;
...
public enum PlayerClass {
    ...
    PlayerClass(final PlayerRace race, final ClassType type, final ClassLevel level) {
        _race = race;
        _level = level;
        _type = type;
    }

    public static EnumSet<PlayerClass> getSet(final PlayerRace race, final ClassLevel level) {
    ...
    public static boolean areClassesComportable(final PlayerClass c1, final PlayerClass c2) {
        if (c1.isOfRace(kamael) != c2.isOfRace(kamael)) {
            return false; // камаэли только с камаэлями
        }
        if (c1.isOfRace(elf) && c2.isOfRace(darkelf) || c1.isOfRace(darkelf) && c2.isOfRace(elf)) {
            return false; // эльфы несовместимы с темными
        }
        if (c1 == Overlord || c1 == Warsmith || c2 == Overlord || c2 == Warsmith) {
            return false; // эти вообще
        }
        if (subclassSetMap.get(c1).equals(subclassSetMap.get(c2))) {
    ...
    public final boolean isOfRace(final PlayerRace race) {
        return _race == race;
    }
