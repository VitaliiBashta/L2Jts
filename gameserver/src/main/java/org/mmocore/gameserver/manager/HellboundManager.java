...
public class HellboundManager {
    private static final Logger LOGGER = LoggerFactory.getLogger(HellboundManager.class);
    private static final long _taskDelay = 2 * 60 * 1000L; //2min
    private static final int NativeHell_native0131 = 19250001; // Kief room
    private static final int NativeHell_native0132 = 19250002;
    private static final int NativeHell_native0133 = 19250003; // Another room
    private static final int NativeHell_native0134 = 19250004;
    private static final int sdoor_trans_mesh00 = 20250002;
    private static final int Hell_gate_door = 20250001;
    private static final int[] _doors = {
    ...
    private HellboundManager() {
        ...
        LOGGER.info("Hellbound Manager: Loaded");
    }
    ...
    public static void setConfidence(final long value) {
        ServerVariables.set("HellboundConfidence", value);
    }

    public static void addConfidence(final long value) {
        ServerVariables.set("HellboundConfidence", Math.round(getConfidence() + value * ServerConfig.RATE_HELLBOUND_CONFIDENCE));
    }

    public static void reduceConfidence(final long value) {
    ...
    public static int getHellboundLevel() {
        final long confidence = ServerVariables.getLong("HellboundConfidence", 0);
        final boolean judesBoxes = ServerVariables.getBool("HB_judesBoxes", false);
        final boolean bernardBoxes = ServerVariables.getBool("HB_bernardBoxes", false);
        final boolean derekKilled = ServerVariables.getBool("HB_derekKilled", false);
        final boolean captainKilled = ServerVariables.getBool("HB_captainKilled", false);

        if (confidence < 1) {
            return AllSettingsConfig.ALLOW_CLOSED_HELLBOUND ? 1 : 0;
    ...
    private void spawnHellbound() {
        ...
        for (final HellboundSpawn hbsi : _list) {
            if (ArrayUtils.contains(hbsi.getStages(), getHellboundLevel())) {
                try {
                    ...
                    for (int i = 0; i < hbsi.getAmount(); i++) {
                        ...
                        spawnDat.doSpawn(true).setParameter("quantity", hbsi.getQuantity());
                        ;
        ...
        LOGGER.info("HellboundManager: Spawned " + _spawnList.size() + " mobs and NPCs according to the current Hellbound stage");
    }
    ...
    private void getHellboundSpawn() {
        ...
        try {
            final File file = new File(ServerConfig.DATAPACK_ROOT + "/data/hellbound_spawnlist.xml");
                ...
                if ("list".equalsIgnoreCase(n1.getNodeName())) {
                    for (Node d1 = n1.getFirstChild(); d1 != null; d1 = d1.getNextSibling()) {
                        if ("data".equalsIgnoreCase(d1.getNodeName())) {
                            counter++;
                            final int npcId = Integer.parseInt(d1.getAttributes().getNamedItem("npc_id").getNodeValue());
                            ...
                            long quantity = 0;
                            if (d1.getAttributes().getNamedItem("quantity") != null) {
                                quantity = Long.parseLong(d1.getAttributes().getNamedItem("quantity").getNodeValue());
                            }

                            final Node att = d1.getAttributes().getNamedItem("stage");
                            final StringTokenizer st = new StringTokenizer(att.getNodeValue(), ";");
                            ...
                            for (Node s1 = d1.getFirstChild(); s1 != null; s1 = s1.getNextSibling()) {
                                if ("territory".equalsIgnoreCase(s1.getNodeName())) {
                                    final CustomPolygon poly = new CustomPolygon(s1.getChildNodes().getLength());
                                    ...
                                    if (!poly.validate()) {
                                        LOGGER.error("HellboundManager: Invalid spawn territory : " + poly + '!');
                                    }
                                }
                            }

                            if (spawnLoc == null && territory == null) {
                                LOGGER.error("HellboundManager: no spawn data for npc id : " + npcId + '!');
                                continue;
                            }

                            final HellboundSpawn hbs = new HellboundSpawn(npcId, spawnLoc, count, territory, respawn, respawnRnd, stages, quantity);
                            _list.add(hbs);
                        }
                    }
                }
            ...
            LOGGER.info("HellboundManager: Loaded " + counter + " spawn entries.");
        } catch (Exception e) {
            LOGGER.warn("HellboundManager: Spawn table could not be initialized.");
            e.printStackTrace();
        }
    }

    private void despawnHellbound() {
        for (final SimpleSpawner spawnToDelete : _spawnList) {
    ...
    private static class DeathListener implements OnDeathListener {
        ...
        public void onDeath(final Creature cha, final Creature killer) {
            ...
            final long quantity = npc.getParameter("quantity", 0);
            ...
            switch (getHellboundLevel()) {
                ...
                case 2: {
                    switch (cha.getNpcId()) {
                        ...
                        case 18464: // Remnant Diviner
                            addConfidence(quantity > 0 ? quantity : 5);
                            break;
                        case 22322: // Subjugated Native
                        case 22323: // Charmed Native
                        case 32299: // Quarry Slave
                ...
                case 3: {
                    switch (cha.getNpcId()) {
                        ...
                        case 22343: // Darion's Executioner
                            addConfidence(quantity > 0 ? quantity : 3);
                            break;
                        case 22341: // Keltas
                            addConfidence(quantity > 0 ? quantity : 100);
                            break;
                        case 22322: // Subjugated Native
                        case 22323: // Charmed Native
                        case 32299: // Quarry Slave
                ...
                case 4: {
                    switch (cha.getNpcId()) {
                        case 18465: // Derek
                            addConfidence(quantity > 0 ? quantity : 10000);
                ...
                case 5: {
                    switch (cha.getNpcId()) {
                        case 22448: // Leodas
                            reduceConfidence(quantity > 0 ? quantity : 50);
                            break;
                    }
                    break;
                }
    ...
    public static class HellboundSpawn {
        ...
        public HellboundSpawn(final int npcId, final Location loc, final int amount, final Territory spawnTerritory, final int respawn, final int respawnRnd, final int[] stages, final long quantity) {
            _npcId = npcId;
            _loc = loc;
            _amount = amount;
