...
public abstract class ResidenceManager extends MerchantInstance {
        ...
        if (actualCommand.equalsIgnoreCase("banish")) {
            ...
            if (val.equalsIgnoreCase("tele")) {
                ...
                for (TeleportLocation loc : locs) {
                    ...
                    teleport_list.append("<a action=\"bypass -h npc_%objectId%_teleport_request ");
                ...
                html.replace("%all%", AllSettingsConfig.ALT_CH_ALL_BUFFS ? "<a action=\"bypass -h npc_%objectId%_support all\">Give all</a><br1><a action=\"bypass -h npc_%objectId%_support allW\">Give warrior</a><br1><a action=\"bypass -h npc_%objectId%_support allM\">Give mystic</a><br>" : "");
                sendHtmlMessage(player, html);
    ...
    private void replace(HtmlMessage html, int type, String replace1, String replace2) {
        ...
        if (getResidence().isFunctionActive(type)) {
            ...
            html.replace('%' + replace1 + "Date%", TimeUtils.dateTimeFormat(getResidence().getFunction(type).getEndDate()));
        } else {
