...
import org.mmocore.gameserver.scripts.npc.model.SepulcherMonsterInstance;
import org.mmocore.gameserver.scripts.npc.model.SepulcherNpcInstance;
import org.mmocore.gameserver.scripts.npc.model.SepulcherRaidInstance;
...
/**
 * TODO[K] - почитать\потестить эту систему, и переделать.
 */
public class FourSepulchersSpawn {
    ...
    private static void loadMysteriousBox() {
        ...
        int count = loadBox(_mysteriousBoxSpawns);
        _log.info("FourSepulchersManager: loaded " + count + " Mysterious-Box spawns.");
    }

    private static int loadBox(Map<Integer, NpcLocation> table) {
        int count = 0;
        try {
                            ...
                            if (spawnType == 0) {
                                for (Node thirdNode = secondNode.getFirstChild(); thirdNode != null; thirdNode = thirdNode.getNextSibling()) {
                                    if ("data".equalsIgnoreCase(thirdNode.getNodeName())) {
                                        ...
                                        table.put(npcKeyId, new NpcLocation(x, y, z, heading, templateId));
                                        count++;
                                    }
                                }
