...
/**
 * @author PaInKiLlEr - AI для Рейб Босса Andreas Van Halter (29062). - Вся информация об АИ расписана. - AI проверен и работает.
 */
public class AndreasVanHalter extends Fighter {
    ...
    protected boolean thinkActive() {
        ...
        if (firstTimeMove) {
            ...
            if (!door1.isOpen() && !door2.isOpen() && door3.isOpen() && door4.isOpen()) {
                ...
                movieTask = ThreadPoolManager.getInstance().schedule(new Movie(1), 3000);
                // Запускаем музыку
                actor.broadcastPacket(new PlaySound(Type.MUSIC, "BS04_A", 1, 0, actor.getLoc()));
                // Вдруг мы не пошли бить РБ, нужно сделать перереспавн и поменять открытие и закрытие дверей местами. Запускаем проверку через 1 час
    ...
    protected void onEvtAttacked(final Creature attacker, final SkillEntry skill, final int damage) {
        ...
        DoorInstance door1 = ReflectionUtils.getDoor(door[2]);
        DoorInstance door2 = ReflectionUtils.getDoor(door[3]);
    ...
    private void DeleteNpc() {
        int[] npcs = {
                TriolsRevelation1, TriolsRevelation2, TriolsRevelation3, TriolsRevelation4, TriolsRevelation5, TriolsRevelation6, TriolsRevelation7, TriolsRevelation8,
                TriolsRevelation9, TriolsRevelation10, TriolsRevelation11, RitualOffering, AltarGatekeeper, AndreasCaptainRoyalGuard1, AndreasCaptainRoyalGuard2,
                AndreasCaptainRoyalGuard3, AndreasRoyalGuards1, AndreasRoyalGuards2, AndreasRoyalGuards3
        };
        ...
        ReflectionUtils.getDoor(door[0]).openMe();
        ReflectionUtils.getDoor(door[1]).openMe();
        // Закрываем двери к алтарю
        ReflectionUtils.getDoor(door[2]).closeMe();
        ReflectionUtils.getDoor(door[3]).closeMe();
    }
        ...
        public void runImpl() {
            ...
            switch (_taskId) {
                case 1:
                    ...
                    actor.doCast(SkillTable.getInstance().getSkillEntry(1168, 7), npc1, false);
                    ...
                    for (Player player : World.getAroundPlayers(actor)) {
                        if (player.getDistance(camera) <= _distance) {
                            ...
                            player.specialCamera(camera, 1500, 88, 89, 0, 5000, 0, 0, 0, 0, 0);
                        } else
                    ...
                    movieTask = null;
                    movieTask = ThreadPoolManager.getInstance().schedule(new Movie(2), 300);
                    break;
                case 2:
                    // Ищем заного жертву
                    NpcInstance npc2 = GameObjectsStorage.getByNpcId(RitualOffering);
                    // Показываем социалку о том что мы умерли
                    npc2.sendPacket(new SocialAction(npc2.getNpcId(), 1));
                    // Стави жертве ХП 0
                    ...
                    if (movieTask != null)
                        movieTask.cancel(false);
                    movieTask = null;
                    movieTask = ThreadPoolManager.getInstance().schedule(new Movie(3), 300);
                    break;
                case 3:
                    // Удаляем жертву
                    ...
                    for (Player player : World.getAroundPlayers(actor)) {
                        if (player.getDistance(camera) <= _distance) {
                            player.enterMovieMode();
                            player.specialCamera(camera, 450, 88, 3, 5500, 5000, 0, 0, 0, 0, 0);
                        } else
                            player.leaveMovieMode();
                    }
                    ...
                    if (movieTask != null)
                        movieTask.cancel(false);
                    movieTask = null;
                    movieTask = ThreadPoolManager.getInstance().schedule(new Movie(4), 9400);
                    break;
                case 4:
                    // Ищем игроков в радиусе и показываем мувик
                    for (Player player : World.getAroundPlayers(actor)) {
                        if (player.getDistance(camera) <= _distance) {
                            player.enterMovieMode();
                            player.specialCamera(camera, 500, 88, 4, 5000, 5000, 0, 0, 0, 0, 0);
                        } else
                            player.leaveMovieMode();
                    }
                    ...
                    if (movieTask != null)
                        movieTask.cancel(false);
                    movieTask = null;
                    movieTask = ThreadPoolManager.getInstance().schedule(new Movie(5), 5000);
                    break;
                case 5:
                    // Ищем игроков в радиусе и показываем мувик
                    for (Player player : World.getAroundPlayers(actor)) {
                        if (player.getDistance(camera) <= _distance) {
                            player.enterMovieMode();
                            player.specialCamera(camera, 3000, 88, 4, 6000, 5000, 0, 0, 0, 0, 0);
                        } else
                            player.leaveMovieMode();
                    }
                    ...
                    if (movieTask != null)
                        movieTask.cancel(false);
                    movieTask = null;
                    movieTask = ThreadPoolManager.getInstance().schedule(new Movie(6), 6000);
                    break;
                case 6:
