...
public class FortuneBug extends DefaultAI {
    ...
    private static final SkillEntry s_display_bug_of_fortune1 = SkillTable.getInstance().getSkillEntry(6045, 1);
    private static final SkillEntry s_display_jackpot_firework = SkillTable.getInstance().getSkillEntry(5778, 1);
    ...
    protected void onEvtArrived() {
        ...
        if (_nextEat < System.currentTimeMillis()) {
            ...
            if (closestItem != null) {
                ...
                ChatUtils.say(actor, 600, NpcString.YUMYUM_YUMYUM);
    ...
    protected void onEvtDead(Creature killer) {
        ...
        if (killer != null) {
            if (i_ai2 == 0) {
                ChatUtils.say(actor, 600, NpcString.I_HAVENT_EATEN_ANYTHING_IM_SO_WEAK);
            } else {
    ...
    @Override
    protected void onEvtTimerFiredEx(int timerId, Object arg1, Object arg2) {
        NpcInstance actor = getActor();
        if (actor == null) {
            return;
        }
        ...
        if (timerId == 7778) {
            switch (i_ai0) {
                case 0:
                    ChatUtils.say(actor, 600, Rnd.chance(50) ? NpcString.IF_YOU_HAVE_ITEMS_PLEASE_GIVE_THEM_TO_ME : NpcString.MY_STOMACH_IS_EMPTY);
                    break;
                case 1:
                    ChatUtils.say(actor, 600, Rnd.chance(50) ? NpcString.IM_HUNGRY_IM_HUNGRY : NpcString.IM_STILL_NOT_FULL);
                    break;
                case 2:
                    ChatUtils.say(actor, 600, Rnd.chance(50) ? NpcString.IM_STILL_HUNGRY : NpcString.I_FEEL_A_LITTLE_WOOZY);
                    break;
                case 3:
                    ChatUtils.say(actor, 600, Rnd.chance(50) ? NpcString.GIVE_ME_SOMETHING_TO_EAT : NpcString.NOW_ITS_TIME_TO_EAT);
                    break;
                case 4:
                    ChatUtils.say(actor, 600, Rnd.chance(50) ? NpcString.I_ALSO_NEED_A_DESSERT : NpcString.IM_STILL_HUNGRY_);
                    break;
                case 5:
                    ChatUtils.say(actor, 600, NpcString.IM_FULL_NOW_I_DONT_WANT_TO_EAT_ANYMORE);
                    break;
            }
            AddTimerEx(7778, 10000 + Rnd.get(10) * 1000);
        } else {
            super.onEvtTimerFiredEx(timerId, arg1, arg2);
        }
    }
