...
public class TreasureChestInstance extends ChestInstance {
    ...
    @Override
    public void onReduceCurrentHp(final double damage, final Creature attacker, SkillEntry skill, final boolean awake, final boolean standUp,
                                  boolean directHp, final boolean lethal) {
        if (!isCommonTreasureChest()) {
            fakeOpen(attacker);
        }
    }
