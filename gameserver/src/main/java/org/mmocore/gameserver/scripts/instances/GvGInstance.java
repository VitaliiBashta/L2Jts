...
public class GvGInstance extends Reflection {
    ...
    public void start() {
        ...
        final ExBlockUpSetState initialPoints = new ExBlockUpSetState(eventTime, team1Score, team2Score);
        final ExBlockUpSetList cui = new ExBlockUpSetList(-1);
        ...
        for (Player tm : HardReferences.unwrap(bothTeams)) {
            ...
            clientSetUp = new ExBlockUpSetState(eventTime, team1Score, team2Score, isRedTeam(tm), tm, 0);
            ...
            broadCastPacketToBothTeams(new ExBlockUpSetList(tm, isRedTeam(tm), 0x01));
        }
    ...
    private void end() {
        ...
        final ExBlockUpSetState end = new ExBlockUpSetState(isRedWinner);
        broadCastPacketToBothTeams(end);

        reward(isRedWinner ? team2 : team1);
        //TODO [VISTALL] GvG.updateWinner(isRedWinner ? team2.getLeader() : team1.getLeader());
        ...
        if (teamId == 1) {
            if (subbing) {
                ...
                if (affectAnotherTeam) {
                    team2Score += toAdd;
                    broadCastPacketToBothTeams(new ExBlockUpSetState(timeLeft, team1Score, team2Score, true, player, getPlayerScore(player)));
                }
                broadCastPacketToBothTeams(new ExBlockUpSetState(timeLeft, team1Score, team2Score, false, player, getPlayerScore(player)));
            } else {
                team1Score += toAdd;
                if (affectAnotherTeam) {
                    team2Score -= toSub;
    ...
    private void removePlayer(Player player, boolean legalQuit) {
        ...
        broadCastPacketToBothTeams(new ExBlockUpSetList(player, isRedTeam(player), 0x02));
        ...
        player.leaveParty();
        if (!legalQuit) {
            player.sendPacket(new ExBlockUpSetState(false));
        }
    ...
    private void teamWithdraw(Party party) {
        if (party == team1) {
            ...
            Player player = team2.getGroupLeader();
            ...
            Player player = team1.getGroupLeader();
