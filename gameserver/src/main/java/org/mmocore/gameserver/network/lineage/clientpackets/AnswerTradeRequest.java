...
import org.mmocore.gameserver.network.lineage.serverpackets.TradeStart;
...
public class AnswerTradeRequest extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (_response == 0) {
            request.cancel();
            requestor.sendPacket(new SystemMessage(SystemMsg.C1_HAS_DENIED_YOUR_REQUEST_TO_TRADE).addString(activeChar.getName()));
            return;
        }

        if (requestor.isActionsDisabled()) {
            request.cancel();
            activeChar.sendPacket(new SystemMessage(SystemMsg.C1_IS_ON_ANOTHER_TASK).addString(requestor.getName()));
            activeChar.sendActionFailed();
            return;
        }
        ...
        try {
            ...
            requestor.sendPacket(new SystemMessage(SystemMsg.YOU_BEGIN_TRADING_WITH_C1).addString(activeChar.getName()), new TradeStart(requestor, activeChar));
            activeChar.setTradeList(new CopyOnWriteArrayList<TradeItem>());
            activeChar.sendPacket(new SystemMessage(SystemMsg.YOU_BEGIN_TRADING_WITH_C1).addString(requestor.getName()), new TradeStart(activeChar, requestor));
        } finally {
            request.done();
        }
    }
