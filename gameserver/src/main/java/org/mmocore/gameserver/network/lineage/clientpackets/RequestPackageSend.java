...
public class RequestPackageSend extends L2GameClientPacket {
    ...
    protected void readImpl() {
        ...
        if (_count * 12 > _buf.remaining() || _count > Short.MAX_VALUE || _count < 1) {
            Log.audit("[PackageSend]", "Player(ID:" + getClient().getActiveChar().getObjectId() + ") name: " + getClient().getActiveChar().getName() + " trying send not correct item count!");
            _count = 0;
            return;
        }
    ...
    protected void runImpl() {
        ...
        if (whkeeper == null || !player.isInRangeZ(whkeeper, player.getInteractDistance(whkeeper))) {
            return;
        }
        ...
        try {
            ...
            slotsleft = OtherConfig.FREIGHT_SLOTS - freight.getSize();
            ...
            final long fee = Math.multiplyExact(items, _FREIGHT_FEE);
            ...
            for (int i = 0; i < _count; i++) {
                ...
                Log.items(player, Log.FreightDeposit, item);
                freight.addItem(item);
            }
        } catch (ArithmeticException ae) {
            Log.audit("[PackageSend]", "Player(ID:" + player.getObjectId() + ") name: " + player.getName() + " exception in main method, m.b used PH!");
            player.sendPacket(SystemMsg.YOU_HAVE_EXCEEDED_THE_QUANTITY_THAT_CAN_BE_INPUTTED);
            return;
        } finally {
            freight.writeUnlock();
