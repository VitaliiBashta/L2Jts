...
public class AnswerCoupleAction extends L2GameClientPacket {
    ...
    private static boolean checkCondition(final Player activeChar, final Player requestor) {
    ...
    protected void runImpl() {
        ...
        switch (_answer) {
            case -1: // refused
                requestor.sendPacket(new SystemMessage(SystemMsg.C1_IS_SET_TO_REFUSE_COUPLE_ACTIONS_AND_CANNOT_BE_REQUESTED_FOR_A_COUPLE_ACTION)
                        .addName(activeChar));
                request.cancel();
                break;
            case 0: // cancel
